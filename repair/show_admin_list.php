...
INNER JOIN user_info ON repair.r_id = user_info.id_card)
INNER JOIN inventory ON repair.serial = inventory.serial)
WHERE r_status='$array_status[1]' OR r_status='$array_status[0]'
ORDER BY r_date DESC";
}
...
<?php
		...
		{	
?>
			<td ><a href="javascript:popup('detail_repair2.php?txt_id=<?php echo $row["repair_id"]; ?>','',800,600)" >[ดูรายละเอียด]</a></td>
<?php 
		} 
		else
		{
...
?>
	...
	<?php
		...
		{
	?>
		<a href="delete_repair.php?txt_id=<?php echo " " . $row["repair_id"] . " "; ?>" onClick="javascript:return confirm('คุณต้องการลบข้อมูลใช่หรือไม่');" >[✖]</a>
	<?php } ?>
			</td>
		</tr>
		</form> 	
